...
function registro($datos){
	...
	$consulta = "SELECT id FROM  `euromillon_login` WHERE  `mail`='".$mailRegistro."'";
	...
	if (mysqli_num_rows($respuesta) != 0) { //Nos devuelve la cantidad de Filas que tiene
		...
		//Hacemos el registro
	$consulta = "INSERT INTO  `euromillon_login` (`id` ,`mail` ,`pass` ,`nombre` ,`apellidos` ,`tel` ,`nivel` ,`activo` ,`debaja`)VALUES (NULL ,  '".$mailRegistro."',  '".$passRegistro."',  '".$nombreRegistro."',  '',  '',  '1',  'no', 'no')";
	$respuesta = getArraySQL($consulta); //Para Enviar consulta
	//Enviar Mail
	//##########################################################
